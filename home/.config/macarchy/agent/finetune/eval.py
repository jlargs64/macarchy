...
HARD = [
    ...
    ("open obsidian and helium, then send helium to space 2",
     [{"name": "launch_app", "arguments": {"app": "obsidian"}}, {"name": "launch_app", "arguments": {"app": "helium"}},
    ...
    ("stack mode and next theme", [{"name": "set_layout", "arguments": {"layout": "stack"}}, {"name": "next_theme", "arguments": {}}]),
    ("split right, then go to tab 3",
     [{"name": "zellij_split_pane", "arguments": {"direction": "right"}}, {"name": "zellij_go_to_tab", "arguments": {"index": 3}}]),
...
def main():
    ...
    print(f"exact match (case-insensitive args): {hits}/{n} = {hits/n:.1%}   median latency {sorted(ms)[n//2]:.0f} ms")
