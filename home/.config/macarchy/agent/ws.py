...
CONNECTORS = re.compile(
    r"\s*(?:,\s*(?:and\s+|then\s+)?|\band then\b|\bthen\b|\band also\b|\bafter that\b|\band\b|\.\s+)\s*", re.I
)
VOCAB = set(
...
def cfg():
    out = {
        "MACARCHY_AGENT_MODEL": os.path.join(HOME, ".local/share/macarchy/models/workspace-agent.cact"),
        "MACARCHY_AGENT_EXECUTE": "false",
        "MACARCHY_MIC": ":default",
...
def voice(c):
    ...
    if os.path.exists(pidf):
        ...
        if ov_pid:
            try:
                os.kill(ov_pid, signal.SIGTERM)
            except ProcessLookupError:
                pass
        os.remove(pidf)
        subprocess.run(
            ["pkill", "-INT", "-f", f"ffmpeg .* {re.escape(wav)}$"], capture_output=True
        )  # strays from any earlier bug
    ...
    if os.path.exists(OVERLAY):  # the pill outlives this process; the next press kills it
        ov = subprocess.Popen(
            [OVERLAY, "listen", "Listening…", "alt-w again to run"],
...
def main(argv):
    ...
    if a.json:
        print(
            json.dumps(
                {"text": text, "reasoning": why, "calls": calls, "commands": [r["cmd"] for r in results]}, indent=1
            )
        )
    else:
        print(f"heard: {text}")
    ...
    if a.voice or a.wav:
        ...
        _ov_say(
            "fail" if failed or not calls else "ok", ("heard: " if execute_mode else "dry-run, heard: ") + text, body
        )
        if _ov:
            _ov.stdin.close()  # the pill fades a few seconds after this
